...
class Record (Base):
    ...
    relationinfo_id = Column('relationinfo_id', Integer, ForeignKey('relationinfo.id'))
...
class Record2person (Base):
    ...
    burried = Column('burried', Boolean)
...
class Person2person (Base):
    ...
    relationinfo = relationship('Relationinfo', foreign_keys=relationinfo_id)
    person = relationship('Person', foreign_keys=p1_id)
    person = relationship('Person', foreign_keys=p2_id)

class Relationinfo (Base):
    __tablename__ = "relationinfo"
    id = Column('id', Integer, primary_key = True)
    name = deferred(Column('name', Text))
    parent_category_id = Column('parent_category_id', Integer, ForeignKey('relationinfo.id'))

    relationinfo = relationship('Relationinfo', foreign_keys=parent_category_id)
...
class Source (Base):
    ...
    id = Column('id', Integer, primary_key = True)
    reference = deferred(Column('reference', Text))
...
class Person2personname (Base):
    ...
    id = Column('id', Integer, primary_key = True)
    person_id = Column('person_id', Integer, ForeignKey('person.id'))
    personname_id = Column('personname_id', Integer, ForeignKey('personname.id'))
